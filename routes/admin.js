...
router.get("/wifi", admin,  (req, res) => {
  wifi.getCurrentConnections((error, network) => {
    if (network.length > 0){ //Conectado a rede, exibe opções de desconexão
      ...
      res.render("admin/wifi", {network: network.slice(0,1), conectado:1})
    }
      ...
      wifi.scan((error, networks) => {
        if (error)
          ...
          res.render("admin/wifi", {network: agrupaWifi(networks), conectado:0})
      });
    }
  });
})
...
function agrupaWifi(networks){
  ...
  networks.forEach(function(network, indice) {
    ...
    if (ssidExistente == 0){ //Acrescenta rede ao novo array
      redesWifi[cont] = {
        ssid: network.ssid,
        security: network.security,
        signal_level: network.signal_level}
...
router.get("/edituser", admin,  (req, res) => {
  Usuario.find().lean().populate("usuarios").sort({login: "asc"}).then((usuarios) => {
...
router.post("/edituser/buscar", admin,  (req, res) => {
  Usuario.find({login:new RegExp(req.body.userLogin, 'i')}).lean().populate("usuarios").sort({login: "asc"}).then((usuarios) => {
